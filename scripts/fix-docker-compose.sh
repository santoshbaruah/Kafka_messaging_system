...
echo -e "${BLUE}=== Fixing Docker Compose File ===${RESET}"

# Check if the docker-compose.enhanced.yaml file exists
if [ ! -f "local-dev/docker-compose.enhanced.yaml" ]; then
    echo -e "${RED}Error: local-dev/docker-compose.enhanced.yaml not found${RESET}"
    exit 1
fi
...
cat > local-dev/docker-compose.enhanced.yaml << 'EOF'
  ...
  kafka-setup:
    ...
    command: >
      bash -c "
        ...
        kafka-topics --bootstrap-server kafka-1:9092 --create --if-not-exists --topic posts --partitions 6 --replication-factor 3
        kafka-topics --bootstrap-server kafka-1:9092 --create --if-not-exists --topic posts.dlq --partitions 6 --replication-factor 3
        echo 'Topics created.'
        exit 0
      "
...
echo -e "${GREEN}Fixed docker-compose.enhanced.yaml${RESET}"
echo -e "${YELLOW}Now restart the environment with:${RESET}"
echo -e "make enhanced-local-dev"
echo -e "${BLUE}=== Docker Compose Fix Complete ===${RESET}"
